...
const htmlTemplate = (collection, component) => `{% spaceless %}
{% set blockId = blockId|default("${collection}_${component}") %}
...
const scssTemplate = (collection, component) => `@import "../index.scss";\n@import "./sprites";

.${collection}_${component} {
...
function createCollection(collectionId, componentId) {
	...
	filesExtensions.map((ext) => {
		...
		let content = "";
		if (ext === "html.twig") content = htmlTemplate(collectionId, componentId);
		else if (ext === "scss") content = scssTemplate(collectionId, componentId);
